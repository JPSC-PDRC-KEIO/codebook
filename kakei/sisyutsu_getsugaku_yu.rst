=============================
支出月額（有配偶）
=============================

総額
===============

質問文
---------------

お宅では、この9月にどのくらい支出、貯蓄（生命保険などの保険料の支払いを含む）、ローン返済をしましたか。０円の場合は、０と記入してください。


.. csv-table:: 支出月額
   :header: 項目, 変数名
   :widths: 15, 15

   生活費その他の支出, :doc:`Q293A </variable/Q293A>`
   貯蓄, :doc:`Q294A </variable/Q294A>`
   ローン返済, :doc:`Q295 </variable/Q295>`


用途
================

.. csv-table:: 生活費その他の支出別、内訳
   :header: 項目, 変数名
   :widths: 15,15


   家族共通の支出, :doc:`Q293B </variable/Q293B>`
   本人のための支出, :doc:`Q293C </variable/Q293C>`
   配偶者のための支出, :doc:`Q293E </variable/Q293E>`
   子どものための支出, :doc:`Q293F </variable/Q293F>`
   それ以外の方のための支出,:doc:`Q293G </variable/Q293G>`

.. warning::
  第2～5回では、本人のための支出のうち「キャリアアップのための支出（資格取得や職業のため）」（ :doc:`Q293D </variable/Q293D>`）を聞いています。

.. csv-table:: 貯蓄別、内訳
   :header: "項目", "変数名"
   :widths: 15, 15

   家族共通の貯蓄, :doc:`Q294B </variable/Q294B>`
   本人のための貯蓄, :doc:`Q294B </variable/Q294B>`
   配偶者のための貯蓄, :doc:`Q294B </variable/Q294B>`
   子どものための貯蓄, :doc:`Q294B </variable/Q294B>`
   それ以外の方のための貯蓄, :doc:`Q294B </variable/Q294B>`


生活費内訳
==================


質問文
----------

次の各項目について、あなたのご家庭でこの9月１ヶ月間に支出した額を記入してください。\
（現金を支払ったものの他、クレジット、ローンで購入したもの、銀行・郵便局から自動的に引き落とした分も含みます）\
（支出がなかったものについては0と記入して下さい）

.. note::
  各細目の記入欄は10万円の位までですが、生活費合計だけ百万円の位まで記入欄を設けています。



.. csv-table::
   :header: 項目, 変数
   :widths: 10, 5

    食料（外食・給食代も含みます）, :doc:`Q879A </variable/Q879A>`
    家賃・地代・住宅の修繕（住宅ローンは含みません）, :doc:`Q879B </variable/Q879B>`
    電気・ガス・水道（上・下）, :doc:`Q879C </variable/Q879C>`
    家具・家事用品（寝具はこちらに含みます）, :doc:`Q879D </variable/Q879D>`
    衣類・履物, :doc:`Q879E </variable/Q879E>`
    保険医療（栄養ドリンク・健康食品も含みます）, :doc:`Q879F </variable/Q879F>`
    交通（自動車購入費用・ガソリン代・定期代なども含みます）, :doc:`Q879G </variable/Q879G>`
    通信（郵便・電話代・インターネットなど）, :doc:`Q879H </variable/Q879H>`
    教育（授業料・受験や補習用の塾・教科書・参考書など）, :doc:`Q879I </variable/Q879I>`
    教養・娯楽（受験・補習塾以外の習い事。教養・娯楽用の耐久消費財はこちらに含みます）, :doc:`Q879J </variable/Q879J>`
    交際, :doc:`Q879K </variable/Q879K>`
    夫・妻・子どもへの小遣い、子どもへの仕送り, :doc:`Q879S </variable/Q879S>`
    親への仕送り、小遣い, :doc:`Q879R </variable/Q879R>`
    その他支出, :doc:`Q879M </variable/Q879M>`


.. warning::
  第6～8回調査では、「親への仕送り、小遣い」を実親（:doc:`Q879O </variable/Q879O>`）と義親（:doc:`Q879N </variable/Q879N>`）分けて聞いており、その他の家族への直接的な支出を「家族への小遣い」（:doc:`Q879L </variable/Q879L>`）として聞いています。

.. csv-table::
   :header: "項目","変数名"
   :widths: 15, 10

    "生活費合計", ":doc:`Q879P </variable/Q879P>`"

.. warning::
   第7回には、合計額のうち地域振興券を使用した額（:doc:`Q887 </variable/Q887>`）も聞いています。

.. toctree::
   :maxdepth: 1
   :hidden:

   /variable/Q293A
   /variable/Q293B
   /variable/Q293C
   /variable/Q293E
   /variable/Q293F
   /variable/Q293G
   /variable/Q294A
   /variable/Q294B
   /variable/Q294C
   /variable/Q294D
   /variable/Q294E
   /variable/Q294F
   /variable/Q295
   /variable/Q293D
   /variable/Q879A
   /variable/Q879B
   /variable/Q879C
   /variable/Q879D
   /variable/Q879E
   /variable/Q879F
   /variable/Q879G
   /variable/Q879H
   /variable/Q879I
   /variable/Q879J
   /variable/Q879K
   /variable/Q879S
   /variable/Q879M
   /variable/Q879R
   /variable/Q879N
   /variable/Q879O
   /variable/Q879P
   /variable/Q887
